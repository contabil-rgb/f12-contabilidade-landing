      ...
      <div className="mt-4 rounded-lg border border-slate-200 p-4 dark:border-gray-700">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-sm font-black text-slate-900 dark:text-gray-100">Contrato mais recente</p>
            <p className="mt-1 max-w-xl truncate text-xs font-semibold text-slate-500 dark:text-gray-300">
              {contratoAtual ? contratoLabel(contratoAtual) : 'Nenhum contrato social enviado'}
            </p>
            {contratoAtual ? (
          ...
          <button
            ...
            className="inline-flex min-w-[8.5rem] items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 transition hover:border-brand-blue hover:text-brand-blue disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700 dark:text-gray-100 dark:hover:border-blue-400 dark:hover:text-blue-200"
          >
        ...
        {contratoAtual ? (
          <div className="mt-3">
            <ContratoSocialActions contrato={contratoAtual} onError={onError} />
          </div>
        ) : null}
        ...
        <button
          ...
          disabled={!contratosOrdenados.length}
          className="mt-4 flex w-full items-center justify-between gap-3 rounded-lg border border-slate-200 px-3 py-2 text-left text-sm font-black text-slate-700 transition hover:border-brand-blue hover:text-brand-blue disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700 dark:text-gray-100 dark:hover:border-blue-400 dark:hover:text-blue-200"
        >
          <span>Histórico de versões ({contratosOrdenados.length})</span>
          <ChevronDown size={16} className={`transition ${historyOpen ? 'rotate-180' : ''}`} aria-hidden="true" />
        </button>

        {historyOpen && contratosOrdenados.length ? (
          <div className="mt-3 space-y-2">
            {contratosOrdenados.map((contrato) => (
              <div
                key={contrato.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-gray-700 dark:bg-gray-800"
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-gray-300">
                    ...
                    {contrato.id === contratoAtual?.id ? ' | Atual' : ''}
                  </p>
                </div>
