...
export function normalizeClienteRowForSync(input, options = {}) {
  ...
  const payload = sanitizeResponsavelEcdByRegime({
    ...
    grupo_empresarial: normalizeNullableText(input?.grupo_empresarial),
    ...
    dias_atraso_texto: normalizeNullableText(input?.dias_atraso_texto ?? input?.dias_atraso),
    ...
    data_entrega_ecf: toIsoDate(input?.data_entrega_ecf),
    data_envio_ecf: toIsoDate(input?.data_envio_ecf),
    responsavel_ecf: normalizeNullableText(input?.responsavel_ecf),
    ...
    pendencias_observacoes: normalizeNullableText(input?.pendencias_observacoes),
