...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'reinf_relatorios_status_envio_check'
      and conrelid = 'public.reinf_relatorios'::regclass
  ) then
    alter table public.reinf_relatorios
      add constraint reinf_relatorios_status_envio_check
      check (status_envio in ('salvo', 'enviado'));
  end if;
end;
$$;
...
declare
  ...
  v_status_envio text;
  v_enviado_em timestamp with time zone;
...
begin
  ...
  v_status_envio := coalesce(nullif(btrim(p_relatorio->>'status_envio'), ''), 'salvo');
  if v_status_envio not in ('salvo', 'enviado') then
    raise exception 'Status de envio do relatorio REINF invalido.';
  end if;

  v_enviado_em := nullif(btrim(p_relatorio->>'enviado_em'), '')::timestamp with time zone;
  if v_status_envio = 'enviado' and v_enviado_em is null then
    v_enviado_em := now();
  end if;
  if v_status_envio = 'salvo' then
    v_enviado_em := null;
  end if;

  v_email_resend_id := nullif(btrim(p_relatorio->>'email_resend_id'), '');
