...
function canSendChecklistReminder(user: PortalUser | null) {
  ...
  return String(user.status ?? "").trim().toLowerCase() === "ativo";
}
...
Deno.serve(async (request) => {
  ...
  if (!to.length || !to.every(isValidEmail)) {
    return jsonResponse({ error: "Destinatario do lembrete invalido." }, 400);
  }

  if (cc.length && !cc.every(isValidEmail)) {
    return jsonResponse({ error: "E-mail em copia invalido." }, 400);
  }

  if (!subject) {
    return jsonResponse({ error: "Assunto do lembrete e obrigatorio." }, 400);
  }

  if (!html && !text) {
    return jsonResponse({ error: "Mensagem do lembrete e obrigatoria." }, 400);
  }
  ...
  if (!response.ok) {
    return jsonResponse({
      error: "Falha ao enviar lembrete pelo Resend.",
