...
alter table public.clientes
  add column if not exists grupo_empresarial text;
...
begin
  ...
  values (
    ...
    nullif(btrim(coalesce(p_cliente->>'grupo_empresarial', '')), ''),
    v_regime,
    ...
    nullif(btrim(coalesce(p_cliente->>'dias_atraso_texto', '')), ''),
...
begin
  ...
  set
    ...
    grupo_empresarial = case when p_cliente ? 'grupo_empresarial' then nullif(btrim(coalesce(p_cliente->>'grupo_empresarial', '')), '') else grupo_empresarial end,
    ...
    dias_atraso_texto = case when p_cliente ? 'dias_atraso_texto' then nullif(btrim(coalesce(p_cliente->>'dias_atraso_texto', '')), '') else dias_atraso_texto end,
