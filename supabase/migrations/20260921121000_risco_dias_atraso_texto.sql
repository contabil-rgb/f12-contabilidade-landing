...
create or replace function public.text_to_numeric_or_zero(value text)
returns numeric
language sql
immutable
as $$
  select case
    when replace(trim(coalesce(value, '')), ',', '.') ~ '^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)$'
...
flags as (
  select
    b.*,
    public.text_to_numeric_or_zero(coalesce(nullif(b.dias_atraso_texto, ''), b.dias_atraso::text)) as dias_atraso_num,
    public.is_text_yes(b.competencia_em_dia) as competencia_em_dia_bool,
    (
      public.is_text_no(b.competencia_em_dia) or
      public.text_to_numeric_or_zero(coalesce(nullif(b.dias_atraso_texto, ''), b.dias_atraso::text)) > 0 or
